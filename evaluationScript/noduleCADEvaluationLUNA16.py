...
def evaluateCAD(seriesUIDs, results_filename, outputDir, allNodules, CADSystemName, maxNumberOfCADMarks=-1,
                performBootstrapping=False,numberOfBootstrapSamples=1000,confidence = 0.95, min_det_thresh=None):
    '''
    ...
    for seriesuid in seriesUIDs:
        ...
        for result in results[1:]:
            ...
            if seriesuid == nodule_seriesuid:
                ...
                # filter probability < min_det_thresh
                if float(nodule.CADprobability) < min_det_thresh:
                    continue
    ...
    for i in range(len(fps_bs_itp)):
        if round(fps_bs_itp[i],3) in froc_list:
            print ('Sensitivity/FPs: {:.3f}, {:.3f}'.format(sens_bs_mean[i], round(fps_bs_itp[i],3)))
            nodOutputfile.write('Sensitivity/FPs: {:.3f}, {:.3f}\n'.format(sens_bs_mean[i], round(fps_bs_itp[i],3)))
            sum_list.append(sens_bs_mean[i])
    
    print('Mean sensitivity: ', np.mean(np.array(sum_list)))
...
#        ax.yaxis.set_ticks(np.arange(0.5, 1, 0.1))
        ...
        plt.savefig(os.path.join(outputDir, "froc_%s.png" % CADSystemName), bbox_inches=0, dpi=100)
    ...
    for seriesuid in seriesUIDs:
        ...
        for annotation in annotations[1:]:
            ...
            if seriesuid == nodule_seriesuid:
                ...
                # filter 3mm < nodule < 30mm 
                if float(nodule.diameter_mm) > 30 or float(nodule.diameter_mm) < 3:
                    continue
...
def noduleCADEvaluation(annotations_filename,annotations_excluded_filename,seriesuids_filename,results_filename, outputDir, min_det_thresh=None):
    '''
    ...
    @param outputDir: output directory
    @param min_det_thresh: minimun detection threshhold
    '''
    ...
    return evaluateCAD(seriesUIDs, results_filename, outputDir, allNodules,
                ...
                numberOfBootstrapSamples=bNumberOfBootstrapSamples, confidence=bConfidence, min_det_thresh=min_det_thresh)


if __name__ == '__main__':
    ################################Need to change this part#######################################################
    # CGH
    annotations_filename = '/home/jennychang/CGH_LungNodule/deepseed/result_CGH/CGH_annotations_iou1_ALD_fixmerge.csv'
    annotations_excluded_filename = '/home/jennychang/CGH_LungNodule/deepseed/result_CGH/annotations_excluded.csv'
    seriesuids_filename = "/home/jennychang/CGH_LungNodule/deepseed/result_CGH/test_list_ALD_0907.csv"
    
    # LUNA16 
    # annotations_filename = '/home/jennychang/CGH_LungNodule/LUNA16_eval/annotations/annotations.csv'
    # annotations_excluded_filename = '/home/jennychang/CGH_LungNodule/LUNA16_eval/annotations/annotations_excluded.csv'
    # seriesuids_filename = '/home/jennychang/CGH_LungNodule/deepseed/result_LUNA16/LUNA16_subset01.csv'
    
    results_filename = '/home/jennychang/CGH_LungNodule/deepseed/result_CGH/ALD_0911_3mm_3anchors_fixmerge/test_150_0907smallthan30/pred_with_gtcls.csv'
    outputDir = '/home/jennychang/CGH_LungNodule/deepseed/result_CGH/ALD_0911_3mm_3anchors_fixmerge/test_150_0907smallthan30/'
