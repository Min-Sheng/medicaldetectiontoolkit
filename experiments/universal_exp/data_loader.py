...
def get_train_generators(cf, logger):
    ...
    if isinstance(cf.root_dir, dict):
        multi_dataset = True
        all_pids_list = np.unique([f"{v['dataset']}_{v['pid']}" for (k, v) in all_data.items()])
    else:
        multi_dataset = False
    ...
    if not os.path.exists(splits_file) and not cf.created_fold_id_pickle:
        fg = dutils.fold_generator(seed=cf.seed, n_splits=cf.n_cv_splits, len_data=len(all_pids_list), file_csv_dict=cf.file_csv_dict, pids_list=all_pids_list, multi_dataset=multi_dataset).get_fold_names()
    ...
    if isinstance(cf.root_dir, dict):
        train_data = {k: v for (k, v) in all_data.items() if any(p == f"{v['dataset']}_{v['pid']}" for p in train_pids)}
        val_data = {k: v for (k, v) in all_data.items() if any(p == f"{v['dataset']}_{v['pid']}" for p in val_pids)}
    else:
...
def get_test_generator(cf, logger):
    ...
    if cf.file_csv_dict:
        if isinstance(cf.root_dir, dict):
            all_pids_list = np.unique([f"{v['dataset']}_{v['pid']}" for (k, v) in test_data.items()])

            train_pids, val_pids, test_pids = [], [], []
            for dataset_name in cf.file_csv_dict.keys():
                train_pids_, val_pids_, test_pids_ = dutils.read_csv(cf.file_csv_dict[dataset_name])
                train_pids.extend([f"{dataset_name}_{idx}" for idx in train_pids_])
                val_pids.extend([f"{dataset_name}_{idx}" for idx in val_pids_])
                test_pids.extend([f"{dataset_name}_{idx}" for idx in test_pids_])
            ...
            test_data = {k: v for (k, v) in test_data.items() if any(p == f"{v['dataset']}_{v['pid']}" for p in test_inter_pids)}
...
def load_dataset(cf, logger, subset_ixs=None, pp_data_path=None, pp_name=None):
    ...
    if isinstance(pp_data_path, dict):
        
        data = OrderedDict()
        for dataset_name in pp_data_path.keys():
        
            p_df = pd.read_pickle(os.path.join(pp_data_path[dataset_name], cf.input_df_name))
            class_targets = p_df['class_target'].tolist()
            pids = p_df.pid.tolist()
            if dataset_name == "lndb":
                pids = [f'{i:04}' for i in pids] 
            imgs = [os.path.join(pp_data_path[dataset_name], '{}_img.npy'.format(pid)) for pid in pids]
            segs = [os.path.join(pp_data_path[dataset_name],'{}_rois.npy'.format(pid)) for pid in pids]

            for ix, pid in enumerate(pids):
                
                targets = [0 for ii in class_targets[ix] if ii > 0]
                data[pid] = {'dataset': dataset_name, 'data': imgs[ix], 'seg': segs[ix], 'pid': pid, 'class_target': targets}
